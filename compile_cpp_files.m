...
if(nargin<1)
    MicrosoftSDKPath=getenv('KINECTSDK10_DIR');
end
if(is64)
...
for i=1:length(files)
    ...
    if(strcmpi(Filename(1:min(end,9)),'mxMSAudio'))
        mex('-v',['-L' MicrosoftSDKPathLib],'-lKinect10','-lMsdmo','-ldmoguids','-luuid','-lamstrmid','-lavrt',['-I' MicrosoftSDKPathInclude],Filename);
    else
        mex('-v',['-L' MicrosoftSDKPathLib],'-lKinect10',['-I' MicrosoftSDKPathInclude],Filename);
    end
end
